...
void timer_periodic_cb(void *arg) //1ms中断一次
{
  ...
  timer_count++;
  if (timer_count >= 10000) //第一次10S等待传感器稳定，第二次5s
  {
    timer_count = 5000;
    ...
    {		
        ...
        uint16_t Temperature_int=Temperature*10;
        uint16_t Humidity_int=Humidity*10;

        bzero(Send811_data,sizeof(Send811_data));
        uint8_t data_len=sprintf(Send811_data,"AT+DTX=24,0267%04X0768%04X80010002\r\n",Temperature_int,Humidity_int);
        ESP_LOGI("811DATATemperature", "%s", Send811_data);
